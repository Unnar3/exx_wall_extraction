...
namespace EXX{
	...
	void compression::extractPlanesRANSAC(int max_iterations, int min_inliers, double distance_threshold)
	{
		...
		while (i < 100 && cloud_->points.size() > 0 && cloud_->points.size() > 0.1 * nr_points)
		{
	...
	void compression::greedyProjectionTriangulation(){
		...
		mesh_ = compression::greedyProjectionTriangulation_s( tmp_cloud );
	}
	...
	void compression::greedyProjectionTriangulationPlanes(){
		...
		for (size_t i = 0; i < sv_planes_.size(); i++){
			*tmp_cloud = *sv_planes_[i]+*rw_hulls_[i];
			meshes_.push_back( compression::greedyProjectionTriangulation_s( tmp_cloud ));
		}
	}

	pcl::PolygonMesh compression::greedyProjectionTriangulation_s(PointCloudT::Ptr cloud){
